...
//    Counters
//
// The first two arguments to all TRACE macros are the category and name. Both
// are strings, and must have application lifetime (statics or literals). The
// same applies to arg_names, and string argument values. However, you can force
// a copy of a string argument value with TRACE_STR_COPY:
//     TRACE_EVENT1("category", "name", "arg1", "literal string is only
//     referenced"); TRACE_EVENT1("category", "name", "arg1",
//     TRACE_STR_COPY("string will be copied"));
//
//
// Categories are used to group events, and
// can be enabled or disabled by the tracing framework. The trace system will
// automatically add the process id, thread id, and microsecond timestamp to all
// events.
//
//
...
//
//
// Trace event also supports counters, which is a way to track a quantity as it
// varies over time. Counters are created with the following macro:
...
// The tracing UI will show these counters in a single graph, as a summed area
// chart.
...
// with 0, 1 or 2 associated arguments. In the framework, the arguments are
// ignored.
...
// This is only used for some shader text logging that isn't supported in ATrace
// anyway.
