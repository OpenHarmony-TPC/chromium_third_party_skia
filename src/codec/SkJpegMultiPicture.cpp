...
std::unique_ptr<SkJpegMultiPictureParameters> SkJpegMultiPictureParameters::Make(
    ...
    for (uint32_t i = 0; i < numberOfImages; ++i) {
        ...
        const uint32_t attribute = get_endian_int(mpEntry->bytes() + 0, littleEndian);
        const uint32_t size = get_endian_int(mpEntry->bytes() + 4, littleEndian);
        const uint32_t dataOffset = get_endian_int(mpEntry->bytes() + 8, littleEndian);
