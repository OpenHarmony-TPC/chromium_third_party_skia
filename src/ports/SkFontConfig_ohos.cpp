...
void FontConfig_OHOS::getAxisValues(const AxisDefinitions& axisDefs,
    ...
    SkFontScanner_FreeType::computeAxisValues(
            axisDefs, position, axisValues, font.familyName, &font.style);
 ...
 */
int FontConfig_OHOS::loadFont(const SkFontScanner& fontScanner, const char* fname) {
    ...
    if (stream == nullptr || fontScanner.scanFile(stream.get(), &count) == false ||
        fontScanner.scanInstance(
    ...
    for (unsigned int i = 0; i < fontDirSet.size(); i++) {
        ...
        while ((node = readdir(dir))) {
            ...
            if (fontDirSet[i][fontDirSet[i].size() - 1] != '/') {
                if (strcat_s(fullname, sizeof(fullname), "/") != EOK) {
                    continue;
                }
            }
            if (strcat_s(fullname, sizeof(fullname), fname) != EOK) {
                continue;
            }
#else
